...
The currently configured OnChainLab router creates uninitialized, non-executable accounts for Loopers and must not be used by this product. Before enabling operator activation, deploy a new immutable `LooperAgentAccount` implementation that is owner-executable immediately after ERC-6551 registry creation, then update only the Looper collection's ERC-6551 implementation setting. ERC-721T metadata and the existing ERC-8004 identity/binding remain unchanged.

The immutable account contract has no initializer, proxy, beacon, upgrade authority, EntryPoint, factory, module registry, arbitrary validator, or delegatecall. It derives `(chainId, tokenContract, tokenId)` from the standard ERC-6551 proxy footer; resolves control dynamically through `ownerOf(tokenId)`; accepts ETH; exposes `owner`, `token`, `state`, EIP-1271 signature validation, ERC-6551 signer validation, and owner-only CALL execution; rejects delegatecall and direct non-owner execution; increments and emits `StateUpdated` before each external execution.

The release pinset records:
...
- deployed `LooperAgentAccount` address, exact runtime bytes/length/SHA-256 and compiler settings generated by the reviewed deployment artifact;
- existing salt `0xff28549509272e76f1d1c6ef7d6976d848c5ff6cb5068b2183c8d52f4cbe2bee`.

Before changing collection configuration, prove the old deterministic #3802 account has no assets and enumerate every old implementation account created for the Looper collection. The configuration transaction is a separate owner-approved release action with an exact transaction preview and canonical post-state verification.
...
### Supported operator-wallet attribution

V1 writes support connected EOAs only. Before enabling activation or sends, require the authenticated operator address to have empty code at the canonical anchor and to equal current `ownerOf(tokenId)`. Smart wallets, delegated EOAs, Coinbase Base Accounts, ERC-4337 wrappers, and unknown wallet profiles remain read-only until a separate reviewed profile defines their exact attribution rules.

Direct activation requires exact chain, sender, registry destination, zero value, exact input, transaction/receipt hash and coordinates, exactly one matching `AccountCreated` event, and canonical post-state.

Direct sends require exact chain, current-owner sender, agent-account destination, zero outer value, exact input, transaction/receipt hash and coordinates, exactly one account `StateUpdated(n+1)` event, pre-state `state()==n`, post-state `state()==n+1`, exact inner call evidence, and the asset-specific post-state evidence below. A matching asset transfer elsewhere in the transaction never establishes attribution.
...
Activation states are the generalized audited states: `prepared`, `submitted`, `uncertain_hashless`, `uncertain_hashed`, `confirmed_attributed`, `observed_unattributed`, `reverted`, `retry_cancelled`, and `superseded`. The exact legal transition/reason table, ten-minute receipt deadline, two-minute confirmation deadline after receipt discovery, three-origin receipt equality, two-block confirmation depth, canonical revalidation, one guarded retry after an undeployed hashless outcome, retry supersession, late-original detection, controls, and acknowledgment behavior carry forward from the audited #3802 tool. Reloaded `prepared`, corrupt storage, receipt timeout, configuration drift, reorg, or incomplete evidence fail closed. If an exact account appears without attributable creation evidence, record `observed_unattributed`; show the verified wallet read-only and do not claim which attempt created it.

Send attempts use `prepared`, `submitted`, `uncertain_hashless`, `uncertain_hashed`, `confirmed_attributed`, `reverted`, and `acknowledged_unknown`. Legal transitions are: `prepared→submitted` on a valid hash; `prepared→uncertain_hashless` on an ambiguous provider result; `prepared→reverted` only on explicit wallet rejection before submission; `submitted→confirmed_attributed|reverted|uncertain_hashed` from canonical evidence; terminal attributed/reverted states may fall back to `uncertain_hashed` on reorg; and either uncertain state may become `confirmed_attributed|reverted` only through later attributable evidence. `acknowledged_unknown` is a display-only terminal lock and never authorizes a retry. They use the same receipt quorum, two-block confirmation, canonical revalidation, reload, cross-tab, and reorg handling. A hashless send has no automatic retry or balance-based success inference. It remains locked unless bounded discovery yields one unique transaction whose exact outer/direct or ERC-4337 envelope, account execution, receipt, and canonical post-state attribute it to the prepared send. Otherwise only a prominent unknown-outcome state is shown.
...
- `packages/contracts/src/LooperAgentAccount.sol`: immutable immediately executable ERC-6551 implementation.
- `packages/contracts/test/looper-agent-account.test.mjs`: compile/deploy/fuzz-style ownership, execution, signature, and transfer-control tests.
- `packages/contracts/scripts/deploy-looper-agent-account.js`: deterministic artifact/deployment preparation and exact collection-config transaction preview; no implicit broadcast.
...
- `apps/web/src/looper-agent-wallet-boundary.js`: purpose-built EOA injected-wallet adapter; no generic request export; smart/delegated accounts are read-only.
...
Permanent tests include the new account contract's fresh-create immediate ETH/ERC-20 execution and owner-transfer control; EOA activation/send attribution; smart/delegated-wallet read-only rejection; cross-tab/reload/storage corruption; receipt timeout/reorg; deployment races; prefunded undeployed accounts; configuration drift at every boundary; old-owner/new-owner transfer behavior; private-memory isolation; malicious token metadata/returns/decimals; provider redirects/caps/deadlines; and upstream-error/log secret leakage.