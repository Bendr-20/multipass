...
(() => {
  ...
  async function sha256Hex(value) {
    const input = hexToBytes(value); const bitLength = BigInt(input.length) * 8n;
    const paddedLength = Math.ceil((input.length + 9) / 64) * 64; const padded = new Uint8Array(paddedLength); padded.set(input); padded[input.length] = 0x80;
    ...
    for (let offset = 0; offset < padded.length; offset += 64) {
      for (let index = 0; index < 16; index += 1) { const at = offset + index * 4; words[index] = ((padded[at] << 24) | (padded[at + 1] << 16) | (padded[at + 2] << 8) | padded[at + 3]) >>> 0; }
      for (let index = 16; index < 64; index += 1) { const x = words[index - 15]; const y = words[index - 2]; const sigma0 = rotateRight32(x, 7) ^ rotateRight32(x, 18) ^ (x >>> 3); const sigma1 = rotateRight32(y, 17) ^ rotateRight32(y, 19) ^ (y >>> 10); words[index] = (words[index - 16] + sigma0 + words[index - 7] + sigma1) >>> 0; }
      let [a,b,c,d,e,f,g,h] = hash;
      for (let index = 0; index < 64; index += 1) { const sum1 = rotateRight32(e, 6) ^ rotateRight32(e, 11) ^ rotateRight32(e, 25); const choice = (e & f) ^ (~e & g); const first = (h + sum1 + choice + SHA256_CONSTANTS[index] + words[index]) >>> 0; const sum0 = rotateRight32(a, 2) ^ rotateRight32(a, 13) ^ rotateRight32(a, 22); const majority = (a & b) ^ (a & c) ^ (b & c); const second = (sum0 + majority) >>> 0; h = g; g = f; f = e; e = (d + first) >>> 0; d = c; c = b; b = a; a = (first + second) >>> 0; }
  ...
  function rotateLane(value, amount) {
    if (amount === 0) return value & KECCAK_MASK;
    const shift = BigInt(amount);
    return ((value << shift) | (value >> (64n - shift))) & KECCAK_MASK;
  }

  function permuteKeccak(state) {
    const column = new Array(5);
    const mixed = new Array(25);
    for (const roundConstant of KECCAK_ROUND_CONSTANTS) {
      for (let x = 0; x < 5; x += 1) column[x] = state[x] ^ state[x + 5] ^ state[x + 10] ^ state[x + 15] ^ state[x + 20];
      for (let x = 0; x < 5; x += 1) {
        const delta = column[(x + 4) % 5] ^ rotateLane(column[(x + 1) % 5], 1);
        for (let y = 0; y < 5; y += 1) state[x + 5 * y] = (state[x + 5 * y] ^ delta) & KECCAK_MASK;
      }
      for (let x = 0; x < 5; x += 1) {
        for (let y = 0; y < 5; y += 1) {
          mixed[y + 5 * ((2 * x + 3 * y) % 5)] = rotateLane(state[x + 5 * y], KECCAK_ROTATIONS[x + 5 * y]);
        }
      }
      for (let x = 0; x < 5; x += 1) {
        for (let y = 0; y < 5; y += 1) {
          state[x + 5 * y] = (mixed[x + 5 * y] ^ ((~mixed[(x + 1) % 5 + 5 * y]) & mixed[(x + 2) % 5 + 5 * y])) & KECCAK_MASK;
        }
      }
  ...
  function hashUserOperationV06(operation) {
    ...
    if (!operation || typeof operation !== 'object' || Array.isArray(operation)) throw new TypeError('UserOperation must be an object.');
    const keys = Object.keys(operation);
    if (keys.length !== fields.length || keys.some((key) => !fields.includes(key)) || fields.some((field) => !Object.hasOwn(operation, field))) {
      throw new TypeError('UserOperation must contain exactly the v0.6 fields.');
    }
    if (!ADDRESS.test(operation.sender)) throw new TypeError('UserOperation sender must be an address.');
    hexToBytes(operation.initCode);
    hexToBytes(operation.callData);
    hexToBytes(operation.paymasterAndData);
    const packed = `0x${addressWord(operation.sender)}${uint256Word(operation.nonce)}${keccak256Hex(operation.initCode).slice(2)}${keccak256Hex(operation.callData).slice(2)}${uint256Word(operation.callGasLimit)}${uint256Word(operation.verificationGasLimit)}${uint256Word(operation.preVerificationGas)}${uint256Word(operation.maxFeePerGas)}${uint256Word(operation.maxPriorityFeePerGas)}${keccak256Hex(operation.paymasterAndData).slice(2)}`;
    const outer = `0x${keccak256Hex(packed).slice(2)}${addressWord(PINSET.identities.entryPoint.address)}${uint256Word(PINSET.chainId)}`;
    return keccak256Hex(outer);
  }
