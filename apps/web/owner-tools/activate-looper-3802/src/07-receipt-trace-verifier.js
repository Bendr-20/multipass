...
(() => {
  ...
  function decodeExecuteBatch(callData) {
    ...
    const body = strip(args); const count = safeNumber(`0x${body.slice(64, 128)}`, 'batch count'); if (count !== 1) throw new Error('Batch must contain exactly one call.'); const relative = safeNumber(`0x${body.slice(128, 192)}`, 'batch tuple offset'); const tupleByte = 64 + relative; const startWord = tupleByte / 32;
  ...
  function decodeUserOperationEvent(receipt, operation, userOpHash) {
    const matches = receipt.logs.filter((log) => log.topics?.[0] === ns.TOPICS.userOperationEvent && log.topics?.[1] === userOpHash && sameAddress(ns.decodeAddress(log.topics?.[2] || '0x'), operation.sender)); if (matches.length !== 1) throw new Error('Expected exactly one selected UserOperationEvent.'); const log = matches[0]; if (ns.hexToBytes(log.data).length !== 128) throw new Error('UserOperationEvent data length mismatch.'); if (ns.decodeUint256(word(log.data, 0)) !== operation.nonce || ns.decodeBool(word(log.data, 1)) !== true) throw new Error('UserOperationEvent nonce or success mismatch.'); return log;
  }
  function findTraceAttribution(trace, selectedCallData, receipt) {
    if (!trace || !sameAddress(trace.to, ns.PINSET.identities.entryPoint.address) || trace.input === undefined) throw new Error('Trace root mismatch.'); const candidates = [];
    function walk(frame, insideSelected = false) {
      const selected = sameAddress(frame.from, ns.PINSET.identities.entryPoint.address) && sameAddress(frame.to, ns.PINSET.identities.sponsor.address) && frame.input === selectedCallData && BigInt(frame.value || '0x0') === 0n && !frame.error;
      if (selected) candidates.push(frame); for (const child of frame.calls || []) walk(child, insideSelected || selected);
    }
    walk(trace); if (candidates.length !== 1) throw new Error('Trace must contain exactly one selected sponsor frame.'); const frame = candidates[0]; const permitted = [ns.PINSET.identities.sponsor.address, ns.PINSET.identities.sponsorDelegate.address, ns.PINSET.identities.sponsorImplementation.address, ns.PINSET.identities.registry.address].map(lower); const registryCalls = [];
    function inspect(node) { for (const child of node.calls || []) { const type = String(child.type || 'CALL').toUpperCase(); if (['CREATE','CREATE2'].includes(type) || (!['CALL','DELEGATECALL','STATICCALL'].includes(type))) throw new Error('Forbidden trace call type.'); if (!permitted.includes(lower(child.to))) throw new Error('Unexpected wallet-envelope external call.'); if (sameAddress(child.to, ns.PINSET.identities.registry.address)) { if (type !== 'CALL' || child.input !== ns.EXACT_CALLDATA || BigInt(child.value || '0x0') !== 0n || child.error) throw new Error('Registry trace call mismatch.'); registryCalls.push(child); } inspect(child); } }
    inspect(frame); if (registryCalls.length !== 1) throw new Error('Trace must contain exactly one registry call.'); const logs = registryCalls[0].logs; if (!Array.isArray(logs)) throw new Error('Trace lacks per-frame logs.'); const matching = logs.filter((log) => log.topics?.[0] === ns.TOPICS.erc6551AccountCreated); if (matching.length !== 1 || !Number.isInteger(matching[0].index) || matching[0].index < 0 || matching[0].index >= receipt.logs.length) throw new Error('Trace registry log ordinal mismatch.'); const receiptLog = receipt.logs[matching[0].index]; if (JSON.stringify({ address: lower(matching[0].address), topics: matching[0].topics, data: matching[0].data }) !== JSON.stringify({ address: lower(receiptLog.address), topics: receiptLog.topics, data: receiptLog.data })) throw new Error('Trace log bytes do not match receipt ordinal.'); return decodeRegistryLog(receiptLog, matching[0].index);
  }
  ...
  async function verifyReceiptEvidence({ requestedHash, transaction, receipt, postState, trace = null, onchainUserOpHashResult = null }) {
    ...
    if (receipt.status === '0x0') return ns.deepFreeze({ classification: postValid ? 'observed_unattributed' : 'reverted', registryLog: null }); if (receipt.status !== '0x1') throw new Error('Receipt status malformed.');
    try { const registryLog = sameAddress(transaction.to, ns.PINSET.identities.registry.address) ? directAttribution(transaction, receipt) : await wrappedAttribution(transaction, receipt, { trace, onchainUserOpHashResult }); if (!postValid) return ns.deepFreeze({ classification: 'uncertain_hashed', registryLog: null }); return ns.deepFreeze({ classification: 'confirmed_attributed', registryLog }); }
