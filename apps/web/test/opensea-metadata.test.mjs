...
test('OpenSea tool metadata publishes a feature image and public tool references', async () => {
  ...
  assert.equal(metadata.external_url, 'https://helixa.xyz/multipass/');
  assert.equal(metadata.properties.kind, 'agent_tool_metadata');
  assert.equal(metadata.properties.api_base_url, 'https://helixa.xyz/api/multipass');
  assert.equal(metadata.properties.profile_route, 'https://helixa.xyz/api/multipass/{agent}');
  assert.equal(metadata.properties.agent_card_route, 'https://helixa.xyz/api/multipass/{agent}/agent-card');
  assert.equal(metadata.properties.tools_route, 'https://helixa.xyz/api/multipass/{agent}/tools');
  assert.equal(metadata.properties.image_alt, 'Helixa AgentDNA OpenSea feature image showing agent identity, public tools, trust signals, and Base AgentDNA registry routes.');
  assert.equal(metadata.properties.subject, 'Helixa AgentDNA');
  const metadataText = JSON.stringify(metadata);
  assert.doesNotMatch(metadataText, /Bendr/i);
  assert.doesNotMatch(metadataText, /bendr-2-1/i);
  assert.doesNotMatch(metadataText, /8453:1/);
  assert.doesNotMatch(metadataText, /Preferred/);
  assert.doesNotMatch(metadataText, /\?agent=1/);
...
test('OpenSea feature image is a dedicated 1200x630 generated asset', async () => {
  ...
  assert.doesNotMatch(svgSource, /Bendr/i);
  assert.doesNotMatch(svgSource, /8453:1/);
  assert.doesNotMatch(svgSource, /Preferred/);
  assert.match(svgSource, /for agents on Multipass/);
