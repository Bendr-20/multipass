...
test('dedicated Console route renders a human-facing operating surface for onchain agents', async () => {
  ...
  assert.match(consolePage.textContent, /Connect your wallet, load an onchain agent, give it a mission, and review its proposals/i);
  assert.ok(consolePage.querySelector('.console-command-strip'));
  assert.equal(consolePage.querySelectorAll('.console-status-strip div').length, 4);
  assert.equal(consolePage.querySelectorAll('.console-flow-panel li').length, 6);
  assert.match(consolePage.textContent, /Next/);
  ...
  assert.equal(consolePage.querySelectorAll('.console-graph-ring').length, 0);
  assert.equal(consolePage.querySelectorAll('.console-graph-lines line').length, 8);
  assert.equal(consolePage.querySelectorAll('.console-graph-node').length, 8);
  assert.match(consolePage.textContent, /Prime/);
  assert.match(consolePage.textContent, /Identity/);
  assert.match(consolePage.textContent, /8453:1/);
  assert.match(consolePage.textContent, /Graph proof/);
  assert.match(consolePage.textContent, /Not published/);
  assert.match(consolePage.textContent, /Public proof/);
  assert.match(consolePage.textContent, /public routes/);
  assert.match(consolePage.textContent, /What it's watching/);
  ...
  assert.match(consolePage.textContent, /You approve every action/i);
...
test('resolver bar renders without changing default static data', async () => {
  ...
  assert.match(resolver.textContent, /Activate an existing agent/);
  assert.match(resolver.textContent, /Enter an AgentDNA ID, ERC-8004-style ID, token ID, or agent name/);
  assert.match(resolver.textContent, /Activate agent/);
