...
test('mobile resolver keeps a compact single-column hierarchy', async () => {
  ...
  assert.match(mobileBlock, /\.profile-visual-strip\s*\{[^}]*max-width:\s*100%;[^}]*overflow:\s*hidden;[^}]*margin-top:\s*22px;/s);
  assert.doesNotMatch(css, /\.visual-card-viewport\s*\{/s);
  assert.doesNotMatch(css, /\.visual-card-track\s*\{[^}]*transform:\s*translate/s);
  assert.match(css, /\.profile-visual-strip \.visual-profile-link\s*\{[^}]*text-decoration:\s*none;/s);
  assert.match(css, /\.profile-visual-strip \.visual-profile-link \.profile-card-visual img\s*\{[^}]*object-fit:\s*contain;/s);
  assert.match(mobileBlock, /\.profile-visual-strip \.card-track\s*\{[^}]*grid-auto-columns:\s*minmax\(0, min\(340px, 88vw\)\);[^}]*overflow-x:\s*auto;[^}]*scroll-snap-type:\s*x proximity;/s);
  assert.match(mobileBlock, /\.profile-visual-strip \.visual-profile-link\s*\{[^}]*min-height:\s*380px;[^}]*grid-template-rows:\s*190px 1fr;[^}]*scroll-snap-align:\s*start;/s);
  assert.match(mobileBlock, /\.profile-visual-strip \.visual-profile-link \.profile-card-visual\s*\{[^}]*min-height:\s*190px;/s);
