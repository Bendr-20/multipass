...
test('Multipass Console snapshot frames onchain agent operations without collection-specific copy', () => {
  ...
  assert.match(snapshot.lead, /load an onchain agent/i);
  assert.match(snapshot.lead, /review its proposals/i);
  ...
  assert.equal(snapshot.status.find((item) => item.label === 'Agent records')?.value, 2);
  ...
  assert.match(snapshot.trustGraph.summary, /who owns the agent/i);
  assert.match(snapshot.trustGraph.summary, /what needs your approval/i);
  ...
  assert.equal(snapshot.trustGraph.nodes.find((node) => node.label === 'Identity')?.state, '8453:1');
  assert.equal(snapshot.trustGraph.nodes.find((node) => node.label === 'Graph proof')?.state, 'Published 8453:18531');
...
test('Multipass Console renderer includes memory missions and market signals as reviewed proposals', () => {
  ...
  assert.equal(root.querySelectorAll('.console-flow-panel li').length, 6);
  ...
  assert.match(text, /Shows who owns the agent/);
  assert.match(text, /Cred tier: Prime/);
  assert.match(text, /What it&#39;s watching|What it's watching/);
...
test('Multipass Console renderer shows fresh-session recall after client reset', () => {
  ...
  assert.match(root.querySelector('.console-trust-graph-card')?.textContent ?? '', /1 saved/);
});
