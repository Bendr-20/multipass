...
test('Multipass Console snapshot frames onchain agent operations without collection-specific copy', () => {
  ...
  assert.match(snapshot.trustGraph.summary, /old-site shape/i);
  assert.match(snapshot.trustGraph.summary, /orbit points show what backs it/i);
...
test('Multipass Console renderer includes memory missions and runtime checks as reviewed proposals', () => {
  ...
  assert.equal(root.querySelectorAll('.console-graph-node').length, 5);
  ...
  assert.equal(root.querySelectorAll('.console-tier-strip span').length, 0);
  assert.equal(root.querySelectorAll('.console-graph-list article').length, 0);
